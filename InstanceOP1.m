classdef InstanceOP1 < PROBLEM
    ...
    methods
        ...
        function Population = Evaluation(obj,varargin)
            load('Instance_1.mat');
            data.nDie      = 10;                                % 晶片的数量
            data.nChainMax = 10;                                % 所允许的最多的测试链
            data.P         = Instance_1.power;                  % 每个晶片对应的测试功率
            data.T         = Instance_1.time;                   % 每个晶片对应的测试时间
            data.Pmax      = 50000;                             % 每个测试组所允许的最大测试功率
            data.Ca        = 0.005;                             % 单位时间的测试成本
            data.Cb        = 993*4.24*1e-8;                     % 每个BIST测试结构的硬件成本
            data.Tmax      = sum(data.T)*0.5;                   % 所允许的最大测试时间
            %data.Tmax      = 1e7;
        ...
        function R = GetOptimum(obj,~)
             R = [421.0320 2234.58]; %HV
            %R = [1.263096e+02,9.892864e+02;1.684128e+02,8.74816e+02;]; % IGD
            % Point1: (993*4.24*1e-8*1e6,sum(data.T)*1/50/1e6*0.005*1e6) 
            % Point2: (993*4.24*1e-8*1e6*nDie,>max(data.T)*1/50/1e6*0.005*1e6)
        end
    end
end
